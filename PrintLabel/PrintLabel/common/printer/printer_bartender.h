#ifndef PRINTER_BARTENDER_H
#define PRINTER_BARTENDER_H

#include <QObject>
#include <QUdpSocket>

struct sBarTend {
    QString pn;
    QString hw;
    QString fw;
    QString sn;
    QString code;
    QString date;
};

class Printer_BarTender : public QObject
{
    Q_OBJECT
    explicit Printer_BarTender(QObject *parent = nullptr, int port = 30044);
public:
    static Printer_BarTender *bulid(QObject *parent = nullptr);
    bool printer(sBarTend &it, int port, int index=0);

protected:
    void delay(int msec);
    bool recvResponse(int sec);
    QString createBaseOrder(sBarTend &it);
    QString createSocketOrder(sBarTend &it);
    QString createNodeOrder(sBarTend &it);
    int sendMsg(const QByteArray &msg, quint16 port,
                const QHostAddress &host=QHostAddress::LocalHost);

protected slots:
    void recvSlot();

private:
    bool mRes;
    int mPort;
    QUdpSocket *mSocket;
};

#endif // PRINTER_BARTENDER_H

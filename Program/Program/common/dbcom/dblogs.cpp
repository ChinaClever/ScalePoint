...
DbLogs::DbLogs()
{
    ...
    tableTile = tr("状态日志");
    //hiddens <<  9;
    headList << tr("操作") << tr("设备序列号") << tr("客户名称") << tr("软件版本")  << tr("结果");
}
...
{
    QString cmd = "insert into %1 (date,time,operation,sn,user,fw,result) "
                  "values(:date,:time,:operation,:sn,:user,:fw,:result)";
...
bool DbLogs::modifyItem(const sLogItem &item, const QString &cmd)
{
    ...
    query.bindValue(":operation",item.op);

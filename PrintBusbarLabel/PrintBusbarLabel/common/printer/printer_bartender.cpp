...
Printer_BarTender::Printer_BarTender(QObject *parent) : QThread(parent)
{
...
void Printer_BarTender::init(const QString &method, const QString &ip, sBarTend &it)
{
    this->mMethod = method;
    this->mIp = ip;
    mIt.on = it.on;
...
void Printer_BarTender::run()
{
    bool ret = false;
    QString str1 = http_post(mMethod, mIp, mIt);
    if(str1 == "Success") {
        ret = true;
    }else {
        str1 = http_post(mMethod, mIp, mIt);
        if(str1 == "Success") {
            ret = true;
        }else ret = false;
    }
